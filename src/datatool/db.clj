...
;(?)collection is clj symbol and item is clj map
(defn add-item! [collection mongo-item]
    (dosync
      (m/insert! collection (endow mongo-item))
      (m/insert! :tags (endow {:name "test-tag"}))))
...
;assumes that items in "data" collections have "datatype_name" as the type field
...
;order preserving with get-column-names, on which it relies
;result format: "url1 url2"
(defn get-column-names-as-string [^java.lang.String datatype_id]
  (let [mongo-datatype (m/fetch-one :datatypes :where {:_id datatype_id})]
    ;(s/trim (apply str (map #(str % " ") (get-column-names datatype_id) ))) ;moved to util
    (u/compact-str-list (get-column-names datatype_id)) ))


;return symbol, e.g. :abc
(defn get-data-mongo-coll-name-for [datatype_id]
  ;currently, we make the assumption that the mongo_coll name is the datatype name
  ;but need TODO a transformation in case the datatype name has spaces, etc!
  ((m/fetch-one :datatypes :where {:_id datatype_id}) :datatype_name))


;TODO: multimethod for :datatype (almost the same as the above)
...
(defn get-mongo-hash-datatype [datatype-fe]
  (let [
    ...
    (map #(str (get % 1)))
	  ...
	  ; mongo_hash:
	  ; {:datatype_name "<some name>", :tags ["tag1" "tag2"],:fields [{:name "url", :type "String"} {:name "url2", :type "String"}], :comments "..." }
	] mongo-hash))

;how do we put this in mongo?
...
(defn get-mongo-hash-datum [datum-fe]
  (let [tags (into [] (.split (datum-fe "tags") " "))
        ...
        ;;final-fields: (it basically gets rid completely of the fields array and types. I guess types can be obtained from the datatype object) {"hello" "b", "some" "a"}  
        ] 
        ;return this
...
;TODO: assumes "datatype_name" is unique identifier -- TO ENSURE
(defn update-datatype-with-fe-item! [datatype-fe]
  (let [fe-item-mongo-hash (get-mongo-hash datatype-fe)
      ...
      (if (nil? existing-mongo-item) ;TODO: use upserts instead of this -- how? (post on mailing list)
        (add-item! type fe-item-mongo-hash)
        ;else:
        (m/update! type existing-mongo-item (merge existing-mongo-item fe-item-mongo-hash)))))
  
  ;IMPORTANT: merging order is important: we are updating with the 2nd arg --in this case, fe-item-mongo-hash; (even if 1st is nil it will work)
  ;TODO: update :tags collection - and how to do that atomically ?
  
; datum-fe: {"type" "bookmark", ...}
...
(defn delete-by-id! [type id]
  (let [mongo-item (m/fetch-one type :where {:_id id})]
    (m/destroy! type mongo-item)));;TODO: handle nil as success (message), and display exceptions
